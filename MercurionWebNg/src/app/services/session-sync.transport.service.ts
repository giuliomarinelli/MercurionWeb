...
export function classifyCrossTabAuthStorageEvent(event: StorageEvent, registry?: BrowserStorageRegistry): CrossTabAuthStorageEvent | null {
  ...
  if (event.storageArea === null) return null
  if (registry && !descriptor) return null
  if (!descriptor && event.key !== 'login' && event.key !== 'ws_accessToken') return null
...
export class SessionSyncTransportService implements OnDestroy {
  ...
  private readonly protocol = new DefaultSessionSyncProtocol()
  ...
  private handleSessionExpired(
    ...
    if (alreadyExpired && !voluntary) return
