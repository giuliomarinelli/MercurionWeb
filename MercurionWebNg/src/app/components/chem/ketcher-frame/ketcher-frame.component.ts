...
export class KetcherFrameComponent implements OnInit, OnDestroy {
  ...
  private smilesResponse$ = new Subject<string>()
  ...
  constructor(
    ...
    effect(() => {
      ...
      if (this._triggerGetSmiles()) {
        this.requestExportSmiles$().pipe(take(1)).subscribe()
  ...
  private onKetcherMessage = (event: MessageEvent) => {
    ...
    if (type === 'smiles') {
      ...
      this.molChange.emit(payload)
      this.exportPolledSmiles.emit(payload)

      this.smilesResponse$.next(payload)
    }
  }
  ...
  private requestExportSmiles$() {
    return defer(() => {
      if (!this.ketcherReady()) return of('')
