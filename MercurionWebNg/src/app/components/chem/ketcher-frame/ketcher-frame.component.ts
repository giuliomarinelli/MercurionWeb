...
export class KetcherFrameComponent implements OnInit, OnDestroy {
  ...
  set smiles(smiles: string | undefined) {
    ...
    // Se Ketcher è pronto, aggiorna subito la molecola
  ...
  constructor(
    ...
    // Polling automatico quando Ketcher è pronto
    effect(() => {
      if (this.ketcherReady()) {
        if (!this.pollHandle) {
          this.pollHandle = setInterval(() => this.requestExportSmiles(), 300);
        }
      } else {
        if (this.pollHandle) {
