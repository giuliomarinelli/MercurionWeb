...
<div class="flex justify-center items-center min-h-screen px-2">
  <div class="w-full max-w-4xl bg-white dark:bg-dark-surface-main rounded-xl shadow-lg">
    ...
    <div class="py-6 px-3 overflow-y-auto flex flex-col gap-4 min-h-[50vh]">
      <!-- body -->
      <p class="my-4 px-4 flex lex-wrap justify-center items-center gap-6">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="fill-current size-10">
          <!--!Font Awesome Pro v7.1.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2025 Fonticons, Inc.-->
        ...
        <span class="text-sm">Seleziona la collezione a cui vuoi aggiungere nuove molecole e clicca su continua. Se vuoi puoi anche crearne una al volo.</span>
      </p>
    ...
    <div class="my-4 mr-8 flex justify-end gap-2">
      ...
      <button
        ...
        (click)="routeAction()"
        [attr.aria-busy]="!selectedCollectionId() || loadingCombo() || loading()"
      >
...
export class SelectCollectionThenRouteComponent implements OnInit, OnDestroy {
  ...
  loadCollections(reset = false) {
    ...
    this.colFetchSub = this.collectionService.getPaginatedCollections(page, 12, this.searchTerm()).subscribe((res) => {
      if (reset) {
  ...
  private goToAddMoleculesToCollection(): void {
    queueMicrotask(() => {
      ...
      this.addToColContext.setRedirectToCollectionPath(this.importFromChembl() === false)
