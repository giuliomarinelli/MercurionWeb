...
@Component({
  ...
  template: `
    @if (_collection()) {
      <article
        class="relative isolate group focus-visible:outline-none cursor-pointer"
      ...
      >
        ...
        >
          ...
          <div class="md:col-span-8 flex items-start gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <div class="text-base md:text-lg font-semibold text-slate-800 dark:text-slate-100 truncate" [title]="_collection()!.name">
                {{ _collection()!.name }}
              </div>
          ...
          <div class="md:col-span-4 flex md:justify-end items-center gap-3 md:gap-4 text-sm text-slate-600 dark:text-slate-300">
            <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 border border-indigo-200/70 dark:border-indigo-700/40 group-hover:scale-[1.02] transition-transform">
              <strong class="mr-1">{{ _collection()!.itemsCount }}</strong>
              {{ _collection()!.itemsCount === 1 ? 'molecola' : 'molecole' }}
            </span>
            <svg class="hidden md:block size-4 opacity-0 group-hover:opacity-100 transition-opacity" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          ...
          <div class="md:col-span-12 mt-1 md:mt-0 flex items-center text-xs text-slate-500 dark:text-slate-400 justify-between">
            ...
            @if (!_isReadonly()) {
              <div class="flex items-center gap-3">
                <button type="button" class="relative z-20 p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors duration-150" title="Duplica collezione" (click)="onActionClick($event); doDuplicateCollection()">
                ...
                <button type="button" class="relative z-20 p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors duration-150" title="Elimina collezione" (click)="onActionClick($event); doDeleteCollection()">
...
export class CollectionCardComponent {
  ...
  @Input() set isReadonly(isReadonly: boolean) { this._isReadonly.set(!!isReadonly); }
