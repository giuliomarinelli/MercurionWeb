...
export class HistoryItemComponent {
  ...
  ariaLabel(): string {
    const dto = this._historyDTO()
    if (!dto) return 'Elemento storico'
    const date = new Date(dto.touchedAt).toLocaleString()
    return `${dto.itemName}. Ultima modifica ${date}`
  }
