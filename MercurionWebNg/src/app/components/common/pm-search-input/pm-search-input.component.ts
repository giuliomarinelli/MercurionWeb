...
@Component({
  ...
  template: `
    <div class="relative w-[240px] mr-2">
      <svg
        class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none fill-current"
      ...
      @if (value.length > 0) {
        <button
          ...
          class="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full
                 hover:bg-slate-200/60 dark:hover:bg-neutral-700 focus:outline-none z-10"
          aria-label="Pulisci">
          <svg class="w-3.5 h-3.5 text-slate-400 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 352 512" aria-hidden="true">
