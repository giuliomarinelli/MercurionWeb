...
@Component({
  ...
  template: `
    <a [routerLink]="_pathToMolecule()" (click)="searchContext.close()" class="flex items-center gap-3 p-3 rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-800 cursor-pointer transition group">
      ...
      <div class="flex-1 min-w-0">
        <div class="text-base font-medium truncate">
          {{ _molecule()?.preferredName }}
        </div>
        <div class="text-xs text-gray-500 truncate">
          {{ _molecule()?.synonyms?.[0] }}
        </div>
