...
@Component({
  ...
  template: `
    @if (canView()) {
      ...
      @switch (step()) {
        @case (1) {
          <h1 id="password-recovery-title" class="text-2xl mt-2 2xs:text-3xl md:text-4xl lg:text-[2.65rem] font-semibold tracking-wider text-center text-light-accent-primary-hc dark:text-dark-accent-primary">
            Recupero password
          </h1>
          ...
          @case (2) {
            <div class="bg-slate-200 dark:bg-slate-800 border my-16 border-slate-300 dark:border-slate-600 relative p-3 mx-auto max-w-[1024px] rounded-md text-sm flex gap-2 xs:gap-4 items-center flex-col xs:flex-row" role="status" aria-live="polite">
              ...
              <span>La password è stata cambiata con successo! <a class="text-light-accent-primary-hc dark:text-dark-accent-primary hover:underline" routerLink="/login">Vai al login</a>.
              </span>
            </div>
          }
      }
