...
function discoverResolverClasses(): Function[] {
  const resolverClasses = discoverResolverFiles(sourceRoot).flatMap((resolverFile) => {
    // The schema command runs through ts-node with tsconfig-paths, so requiring
    // source files registers exactly the same decorator metadata used by Nest.
...
async function main(): Promise<void> {
  const unexpectedArguments = process.argv.slice(2).filter((argument) => argument !== updateFlag)
