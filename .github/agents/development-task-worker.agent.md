...
2. If the recipe requires browser/runtime evidence, perform the capability preflight from `RUNTIME.md` before editing: start the task-scoped runtime, prove the nginx edge and required public state are usable through the dedicated persistent Chrome profile, and, when authentication is required, activate the deterministic local dummy fixture through `http://localhost:8888/__local/dummy-auth` before proving the protected state. Then stop every runtime process you started. If readiness or fixture activation is unavailable, make no task change, leave all outcome checkboxes untouched, and return `SESSION_CAPABILITY_PAUSE`.
