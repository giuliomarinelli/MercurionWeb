...
npm run start --workspace mercurion\_web\_node
...
npm run start\:dev --workspace mercurion\_web\_node
...
npm run start\:debug --workspace mercurion\_web\_node
...
plus \`mercurion-browser-runtime\` whenever browser/runtime evidence is required.

Browser tooling is capability-based rather than host-specific: use any available

approved browser-control path, including \`chrome-devtools\` MCP, Playwright MCP,

the ChatGPT Chrome/browser extension, or direct interactive browser control.

The nonce capability probe invokes no tools, including
...
\- Series membership is owned by the series document through its inclusive YAML \`task\_range\`; task files do not need to encode a backlink to their series.

\- \`card\_id\` is series metadata used to bind a series to its Trello card. It is not an execution dependency.
...
\- \`SKIPPED\_DEPENDENCY\`: never attempted because a hard prerequisite is terminal non-\`DONE\`; never create a feature branch.

All four unchecked means pending. At most one may be checked. \`CI\_PENDING\`,

\`WAITING\_DEPENDENCY\`, \`SESSION\_CAPABILITY\_PAUSE\`,

\`SESSION\_BRANCH\_COLLISION\_PAUSE\`, and \`SESSION\_RECOVERY\_PENDING\` are transient
...
\`SKIPPED\_DEPENDENCY\` merely because it was excluded from autonomous execution.
...
\`authorized\_recovery\` entry containing the exact pending task, Source, existing

feature branch and preserved SHA. The worker receives \`recovery\_resume: true\`,
...
branch collisions enter \`SESSION\_BRANCH\_COLLISION\_PAUSE\`; unsafe shared-state

problems enter \`SESSION\_RECOVERY\_PENDING\`. The coordinator continues other safe

independent work or retries recovery with bounded backoff. It calls

\`task\_complete\` only at the soft deadline or genuine workload exhaustion after
...
Before creating a task branch, the coordinator MUST also make exactly one session-level, non-mutating synchronous \`task\` handshake using \`agent\_type: development-task-worker\`, \`capability\_probe: true\`, and a fresh nonce. The worker returns exactly \`TASK\_CAPABILITY\_OK \<nonce>\` without invoking tools or touching the repository. Empty, denied, malformed, or mismatched delegation prevents task dispatch, enters \`SESSION\_RECOVERY\_PENDING\`, and is retried with bounded backoff until restored or the soft deadline.

The coordinator MUST also verify the effective repository-local \`commit.gpgSign=false\`. Every autonomous commit-producing command uses \`--no-gpg-sign\`, including ordinary commits, no-fast-forward merges, and reverts. A denied install, network, filesystem, cleanup, GitHub, \`task\`, MCP, signing, or \`task\_complete\` prerequisite is reported exactly, enters \`SESSION\_RECOVERY\_PENDING\`, and is retried without weakening the prerequisite or finalizing early.
...
Enter \`SESSION\_RECOVERY\_PENDING\` and keep retrying safe baseline verification
...
unpublished attempt branch when safe and enter \`SESSION\_RECOVERY\_PENDING\` as a
...
7\. Mark the task \`DONE\` in the feature branch only when implementation and all local gates pass. The runner MUST treat this state as \`CI\_PENDING\` until both feature and post-merge CI succeed.
...
\`WAITING\_DEPENDENCY\`; a recipe whose hard-prerequisite chain contains a

terminal non-\`DONE\` outcome belongs to the terminal skip closure. Materialize

all newly affected descendants as \`SKIPPED\_DEPENDENCY\` in one aggregate
...
\`SESSION\_RECOVERY\_PENDING\`: suspend new task dispatch, preserve all branches,
...
Allowed task/session-lifecycle writes include ordinary branch creation, add/commit, push, no-ff merge into \`develop\`, merge revert after failed CI, branch deletion after successful CI, metadata-only commits needed to record \`BLOCKED\`, \`REVERTED\`, or \`SKIPPED\_DEPENDENCY\`, and the final session-report commit.
...
Browser validation is capability-based and MUST NOT depend on one specific MCP

server, browser automation stack, or browser profile. GitHub Copilot CLI may use

the repository's \`chrome-devtools\` MCP server from \`.github/mcp.json\`, while

other supported agents or manual/human-led recovery sessions may use Playwright

MCP, the ChatGPT Chrome/browser extension, direct interactive browser control, or

another available browser-testing integration that can establish the required

evidence.

Any browser profile may be used for non-production testing when it is useful for

the active task, including a dedicated automation profile, an isolated profile,

Incognito/Guest where appropriate, or the developer's existing Chrome profile.

An agent MUST NOT refuse browser validation solely because the available tooling

uses the developer's profile, relies on an installed browser extension, or does

not expose Chrome DevTools MCP. Browser/runtime evidence is judged by the quality

of the evidence produced, not by the profile or control transport used.
...
\- when the active task declares browser validation or runtime browser behaviour is necessary to establish an acceptance criterion, use any available browser-control mechanism capable of producing the required evidence. Chrome DevTools MCP, Playwright MCP, the ChatGPT Chrome/browser extension, and direct interactive browser control are all permitted; no one mechanism is mandatory when an equivalent capability is available;
...
\- select the browser profile and browser-control mechanism that best fit the task and the capabilities available in the current agent host. Attaching to the developer's existing browser profile is explicitly permitted for non-production validation, including when its installed extensions or existing non-production authenticated state are needed for the test;

\- avoid concurrent automation against the same browser profile when it could make evidence nondeterministic or corrupt shared browser state. The coordinator and workers may use different profiles or browser-control mechanisms when necessary, but each validation step must retain clear ownership of the state it is asserting;

\- stored non-production authentication in an explicitly selected browser profile may be reused when appropriate. A fresh login is required only when the task's acceptance criteria require proving the login flow, when the existing state is invalid, or when the selected profile does not already have the required authenticated state;

\- credentials and form data may be entered through any supported browser-control mechanism, including Chrome DevTools MCP, Playwright MCP, the ChatGPT Chrome/browser extension, or ordinary interactive browser input. Do not expose credentials in logs, task reports, screenshots, repository files, or source control;
...
\- a task that explicitly tests logout may leave the selected profile anonymous; later work may reuse that profile, select another profile, or log in again as appropriate, so anonymous state alone never emits \`BROWSER_PROFILE_RECOVERY_REQUIRED\` or stops later task selection;

\- never browse production or enter production credentials/data during autonomous validation;

\- if one browser-control path is unavailable, denied, unsupported by the current agent host, or missing a required extension/MCP server, try another permitted browser-control path before returning \`SESSION_CAPABILITY_PAUSE\`. If task-caused changes or an acceptance-specific post-implementation problem prevent required validation across the available permitted mechanisms, mark the task \`BLOCKED\` rather than claiming browser validation passed.
