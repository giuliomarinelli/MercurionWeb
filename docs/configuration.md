...
`APP_AES_SECRET` is deployment-owned root material. OAuth provider credentials
derive a purpose-specific key and follow the versioned envelope, rotation and
recovery contract in [OAuth provider credential policy](security/oauth-provider-credential-policy.md).
| Support | `APP_SUPPORT_EMAIL` | yes | Non-secret support address. |
| PostgreSQL | `SQL_DATABASE_TYPE`, `SQL_DATABASE_HOST`, `SQL_DATABASE_PORT`, `SQL_DATABASE_USERNAME`, `SQL_DATABASE_PASSWORD`, `SQL_DATABASE`, `SQL_DATABASE_LOGGING`, `SQL_DATABASE_LOGGER` | yes | Database connection and logging; password is secret. The [database schema](database-schema.md) is externally managed and is never created or mutated by this application. |
