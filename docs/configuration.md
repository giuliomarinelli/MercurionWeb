...
| App/NATS | `APP_PORT`, `APP_NATS_PORT`, `APP_NATS_HOST`, `APP_PROJECT_NAME`, `APP_GLOBAL_NAME`, `APP_PROJECT_ID`, `APP_CORS_ORIGINS`, `APP_USER_ACTIVATION_ORIGIN`, `APP_HOST`, `APP_MAX_NATS_PAYLOAD_BYTES`, `APP_SHUTDOWN_TIMEOUT_MS` | mostly required | Non-secret listener, identity, origin and NATS transport settings. |
...
| PostgreSQL | `SQL_DATABASE_TYPE`, `SQL_DATABASE_HOST`, `SQL_DATABASE_PORT`, `SQL_DATABASE_USERNAME`, `SQL_DATABASE_PASSWORD`, `SQL_DATABASE`, `SQL_DATABASE_SYNCHRONIZE`, `SQL_DATABASE_LOGGING`, `SQL_DATABASE_LOGGER` | yes | Database connection and TypeORM behavior; password is secret. |
