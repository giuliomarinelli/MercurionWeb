# Mercurion Code Red — overweek autonomous full-Series launch v7
...
`2026-09-20T10:00:00+02:00` (Europe/Rome, CEST), nove giorni dopo la
...
is 0060. Continue serially through eligible tasks until the soft deadline or
...
Tasks through 0059 that are marked `DONE` are terminal; never select them again.
Select task 0060 first, then continue with the next independent READY task from
...
and finish the original recipe. Task 0059/UI-001 is already integrated as
`DONE`; its archived attempt remains historical evidence and must not be
reopened or selected by this session.
