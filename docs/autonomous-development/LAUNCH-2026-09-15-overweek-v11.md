# Mercurion Code Red — overweek autonomous full-Series launch v11
...
The current active snapshot contains 157 `DONE`, 42 `PENDING`, 4 `BLOCKED`,
12 `SKIPPED_DEPENDENCY` and no `REVERTED` recipes. Five reserved
...
is 0081. Continue serially through eligible tasks until the soft deadline or
...
Task 0195/QA-009 is now `DONE`; seven stale dependency skips (`0197`, `0198`,
`0214`, `0215`, `0217`, `0218` and `0219`) identified by the authoritative
planner were reset to `PENDING`. Let the planner determine `READY` versus
...
branches `feature/UI-014`, `feature/BE-012`, `feature/BE-033` and
`feature/DATA-030`, plus the local deferred-Notebook `feature/SYS-020` ref;
