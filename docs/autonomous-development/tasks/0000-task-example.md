...
> This file is documentation and a template for humans or models that create
> autonomous-development task specifications. It is never part of an executable
> workload. Copy it to a new file whose numeric prefix starts at `0001`.

## Objective

State one concrete implementation outcome in a few sentences.

The objective should describe **what must be true when the task is finished**, not
merely an activity such as "investigate" or "work on" something unless
...
> are retried with bounded exponential backoff and permanent failures are sent to
> the existing dead-letter flow.
...
List likely starting points. This is guidance, not necessarily an exhaustive list.
