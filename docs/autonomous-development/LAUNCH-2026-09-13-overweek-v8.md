# Mercurion Code Red — overweek autonomous full-Series launch v8
...
after 2026-09-21T10:00:00+02:00. Require a clean, synchronized develop and a
...
0077/UI-019, 0109/NG-023, 0114/NG-028, 0120/BE-006, 0136/BE-022 and
0150/DATA-001 from their exact preserved feature branches. For each, verify the
configured local and remote SHA, dispatch the worker with
`recovery_resume: true`, merge current green develop into the feature branch
without rebase/history rewriting, preserve coherent prior work and finish the
original recipe. Reopening a recipe does not supply a missing product,
transaction, trust-policy or schema-baseline decision: each worker must still
honor the original stop conditions.

All 79 former `SKIPPED_DEPENDENCY` recipes and the six former terminal roots
were reset to `PENDING` by direct owner instruction on 2026-09-13. Let the
planner determine `READY` versus `WAITING_DEPENDENCY`; do not rematerialize a
skip unless a root reaches a new persistent non-`DONE` outcome during this
session.
