...
const paths = {
  ...
  preparedSession: 'docs/autonomous-development/session.overweek-2026-09-24-v14.yaml',
  ...
  preparedLaunch: 'docs/autonomous-development/LAUNCH-2026-09-17-overweek-v14.md',
...
for (const [pattern, message] of [
  [/docs\/autonomous-development\/session\.overweek-2026-09-24-v14\.yaml/, 'prepared launch must reference its dated session configuration'],
  ...
  [/expected first READY task[\s\S]{0,20}(?:is\s*)?0082/i, 'prepared launch must state the current expected first ready task'],
  [/No pending recipe has an `authorized_recovery` entry/i, 'prepared launch must forbid implicit recovery of stale branches'],
  [/Tasks `0072\/UI-014`[\s\S]*`0218\/QA-032` are `BLOCKED`[\s\S]*11 recipes currently marked `SKIPPED_DEPENDENCY`[\s\S]*explicitly deferred until after this session/i, 'prepared launch must preserve all current blockers and defer their recovery'],
...
for (const [pattern, message] of [
  ...
  [/name:\s*mercurion-code-red-0001-overweek-full-series-2026-09-24-v14/, 'prepared session must use a fresh session identity'],
  ...
  [/expected_current_done:\s*185/, 'prepared workload must record 185 DONE tasks'],
  ...
  [/expected_current_pending:\s*12/, 'prepared workload must record 12 pending tasks'],
  [/expected_first_ready_task:\s*"0082"/, 'prepared workload must start from task 0082'],
  [/expected_planner_ready:\s*2/, 'prepared workload must record 2 ready tasks'],
  [/expected_planner_waiting_dependency:\s*10/, 'prepared workload must record 10 waiting tasks'],
  [/tasks:\s*\[\]/, 'prepared workload must select the complete Series'],
  [/expected_autonomous_pending:\s*12/, 'prepared workload must record all 12 active pending tasks in scope'],
  ...
  [/blocked_branch_policy:[\s\S]*branches:[\s\S]*- feature\/UI-014[\s\S]*- feature\/NG-016[\s\S]*- feature\/DATA-030[\s\S]*- feature\/QA-023[\s\S]*- feature\/QA-028[\s\S]*- feature\/QA-032[\s\S]*- feature\/SYS-020/, 'prepared session must preserve all frozen and deferred branches'],
