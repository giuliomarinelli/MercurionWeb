...
const paths = {
  ...
  preparedSession: 'docs/autonomous-development/session.overweek-2026-09-23-v13.yaml',
  ...
  preparedLaunch: 'docs/autonomous-development/LAUNCH-2026-09-16-overweek-v13.md',
...
for (const [pattern, message] of [
  [/docs\/autonomous-development\/session\.overweek-2026-09-23-v13\.yaml/, 'prepared launch must reference its dated session configuration'],
  ...
  [/expected first READY task[\s\S]{0,20}(?:is\s*)?0171/i, 'prepared launch must state the current expected first ready task'],
  [/No pending recipe has an `authorized_recovery` entry/i, 'prepared launch must forbid implicit recovery of stale branches'],
  [/Tasks `0072\/UI-014`[\s\S]*`0209\/QA-023` are `BLOCKED`[\s\S]*22 recipes currently marked `SKIPPED_DEPENDENCY`[\s\S]*explicitly deferred until after this session/i, 'prepared launch must preserve all current blockers and defer their recovery'],
...
for (const [pattern, message] of [
  ...
  [/name:\s*mercurion-code-red-0001-overweek-full-series-2026-09-23-v13/, 'prepared session must use a fresh session identity'],
  ...
  [/expected_current_done:\s*173/, 'prepared workload must record 173 DONE tasks'],
  [/expected_current_blocked:\s*7/, 'prepared workload must record seven blockers'],
  [/expected_current_reverted:\s*0/, 'prepared workload must record zero reverted tasks'],
  [/expected_current_skipped_dependency:\s*22/, 'prepared workload must record 22 terminal skips'],
  [/expected_current_pending:\s*13/, 'prepared workload must record 13 pending tasks'],
  [/expected_first_ready_task:\s*"0171"/, 'prepared workload must start from task 0171'],
  [/expected_planner_ready:\s*5/, 'prepared workload must record 5 ready tasks'],
  [/expected_planner_waiting_dependency:\s*8/, 'prepared workload must record 8 waiting tasks'],
  [/tasks:\s*\[\]/, 'prepared workload must select the complete Series'],
  [/expected_autonomous_pending:\s*13/, 'prepared workload must record all 13 active pending tasks in scope'],
  ...
  [/blocked_branch_policy:[\s\S]*branches:[\s\S]*- feature\/UI-014[\s\S]*- feature\/UI-023[\s\S]*- feature\/NG-016[\s\S]*- feature\/BE-012[\s\S]*- feature\/BE-033[\s\S]*- feature\/DATA-030[\s\S]*- feature\/QA-023[\s\S]*- feature\/SYS-020/, 'prepared session must preserve all frozen and deferred branches'],
...
if (/^authorized_recovery:/m.test(preparedSession)) {
  fail(paths.preparedSession, 'must not retain stale recovery authority without matching preserved branches');
}
