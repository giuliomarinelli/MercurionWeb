...
const paths = {
  ...
  preparedSession: 'docs/autonomous-development/session.overweek-2026-09-28-v16.yaml',
  ...
  preparedLaunch: 'docs/autonomous-development/LAUNCH-2026-09-20-overweek-v16.md',
...
for (const [pattern, message] of [
  [/docs\/autonomous-development\/session\.overweek-2026-09-28-v16\.yaml/, 'prepared launch must reference its dated session configuration'],
  [/2026-09-28T10:00:00\+02:00/, 'prepared launch must retain the exact soft deadline'],
  ...
  [/expected first READY task[\s\S]{0,20}(?:is\s*)?0112/i, 'prepared launch must state the current expected first ready task'],
  [/No pending recipe has an `authorized_recovery` entry/i, 'prepared launch must forbid implicit recovery of stale branches'],
  [/no recipes currently marked `BLOCKED`, `REVERTED` or[\s\S]*`SKIPPED_DEPENDENCY`/i, 'prepared launch must record the absence of terminal non-DONE outcomes'],
...
const preparedDeadline = '2026-09-28T10:00:00+02:00';
const preparedDeadlineMatches = preparedSession.match(
  /^\s*end:\s*"2026-09-28T10:00:00\+02:00"/gm,
);
...
for (const [pattern, message] of [
  ...
  [/name:\s*mercurion-code-red-0001-overweek-full-series-2026-09-28-v16/, 'prepared session must use a fresh session identity'],
  ...
  [/expected_current_done:\s*210/, 'prepared workload must record 210 DONE tasks'],
  ...
  [/expected_current_pending:\s*5/, 'prepared workload must record 5 pending tasks'],
  [/expected_first_ready_task:\s*"0112"/, 'prepared workload must start from task 0112'],
  [/expected_planner_ready:\s*4/, 'prepared workload must record 4 ready tasks'],
  [/expected_planner_waiting_dependency:\s*1/, 'prepared workload must record 1 waiting task'],
  [/tasks:\s*\[\]/, 'prepared workload must select the complete Series'],
  [/expected_autonomous_pending:\s*5/, 'prepared workload must record all 5 active pending tasks in scope'],
  ...
  [/blocked_branch_policy:[\s\S]*branches:\s*\[\]/, 'prepared session must record that no frozen recovery branches remain'],
